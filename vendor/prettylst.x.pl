...
my %conversion_enable =
	...
	'No reformat message'	=> 0,			#[ gozzilioni ] remove message of reformatting
	'Missing spell DC'	=> 0,			#[ gozzilioni ] check for missing DC on spells
	'System pcc in data'	=> 0,			#[ gozzilioni ] some system files are data driven
...
my %cl_options = (
	...
	vendor_path		=> q{},	# Path for the input directory
...
if ( scalar @ARGV ) {
	GetOptions(
		...
		'vendorpath|v=s'		=> \$cl_options{ vendor_path },
...
my %master_file_type = (
	...
	SIZE => [
		...
		{ Linetype	=> 'SIZE',
			...
			Header		=> BLOCK_HEADER,
			ValidateKeep	=> YES,
			RegExIsMod		=> qr(^(.*)\.(MOD|FORGET|COPY=[^\t]+)),
			RegExGetEntry	=> qr(SIZENAME:(.*)),
		},
	],
...
if ($cl_options{input_path}) {
	...
	if ( $conversion_enable{'Export lists'} ) {
		...
		open $filehandle_for{CLASS}, '>', 'G_export_class.csv';
		print { $filehandle_for{CLASS} } qq{"Class Name","Abb","Spellcaster","Type","Sourcepage","Line","Filename"\n};

		# Will hold the list of all deities found in DEITY filetypes
		open $filehandle_for{DEITY}, '>', 'G_export_deity.csv';
		print { $filehandle_for{DEITY} } qq{"Deity Name","Pantheon","Sourcepage","Line","Filename"\n};

		# Will hold the list of all domains found in DOMAIN filetypes
		open $filehandle_for{DOMAIN}, '>', 'G_export_domain.csv';
		print { $filehandle_for{DOMAIN} } qq{"Domain Name","Sourcepage","Line","Filename"\n};

		# Will hold the list of all equipements found in EQUIPMENT filetypes
		open $filehandle_for{EQUIPMENT}, '>', 'G_export_equipment.csv';
		...
		open $filehandle_for{RACE}, '>', 'G_export_race.csv';
		print { $filehandle_for{RACE} } qq{"Race Name","Race Type","Race Subtype","Sourcepage","Line","Filename"\n};

		# Will hold the list of all skills found in SKILL filetypes
		open $filehandle_for{SKILL}, '>', 'G_export_skill.csv';
		print { $filehandle_for{SKILL} } qq{"Skill Name","Sourcepage","Stat","Line","Filename"\n};
 
		# Will hold the list of all spells found in SPELL filetypes
		open $filehandle_for{SPELL}, '>', 'G_export_spell.csv';
		print { $filehandle_for{SPELL} } qq{"Spell Name","Classes","Domain","Descriptor","Type","Source Page","Line","Filename","Desc","Save"\n};
		...
		# Will hold the list of all weaponprof found in Proficiency filetypes
		...
		open $filehandle_for{VARIABLE}, '>', 'G_export_variable.csv';
		...
		open $filehandle_for{Willpower}, '>', 'G_export_willpower.csv';
	...
	for my $pcc_file_name ( sort @filelist ) {
		...
		if ($tag) {
			if ( $validfiletype{$tag} ) {
				...
				$value =~ s/^([^|]*).*/$1/;
				my $lstfile = find_full_path( $value, $currentbasedir, $cl_options{basepath}, $cl_options{vendor_path} );
		...
		# } 
			...
			if (!$must_write) {
				$logging->ewarn( DEBUG, qq{Not written because of mustwrite value $must_write}, $pcc_file_name );
			}
			if (!$writefiletype{"PCC"} ) {
				$logging->ewarn( DEBUG, qq{Not written because of writefiletype value $writefiletype{"PCC"}}, $pcc_file_name );
			}
		}
	}
...
if ( $conversion_enable{'System pcc in data'} ) {
	...
	for my $file_name ( sort keys %ex_system_files ) {
		if ( $file_name =~ /$cl_options{input_path}/xmsi ) {
...
for my $file (@files_to_parse_sorted) {
	...
	if ( $file eq "STDIN" ) {
		...
		# We read only files located in input_path dir
		next FILE_TO_PARSE if ($file !~ /$cl_options{input_path}/xmsi );
...
#		} split '\|', $tag_value;
	...
	if (   $conversion_enable{'ALL:Add TYPE=Base.REPLACE'}
		...
		if ( $1 ne 'BASEAB' ) {
		$_[1] =~ s/\|baseab\|/\|BASEAB\|/i;
		$logging->ewarn( WARNING,
			qq{Changing "$tag_name$tag_value" to "$_[0]$_[1]" (BASEAB must be in uppercase)},
			$file_for_error,
			$line_for_error
		);
		$tag_value = $_[1];
...
#			delete $line_ref->{'CSKILL'};
			...
			&& ( exists $line_ref->{'CLASSES'} ) )
		{
			...
			for ( split '\|', $line_ref->{'CLASSES'}[0] ) {
				...
				$spell_level = (/=(\d+)$/)[0] if  (/=(\d+)$/) < $spell_level;
				last CLASS;
				}
			}
		...
		&& $filetype eq 'RACE'
		&& $line_info->[0] eq 'RACE'
		&& $cl_options{convert} eq 'Gozzilioni') {
			my ( $dir, $race );
			$dir  = File::Basename::dirname($file_for_error);
			$race = $line_ref->{ $master_order{'RACE'}[0] }[0];
			record_bioset_tags( $dir, $race, "", "", "", $file_for_error, $line_for_error );
		}
		...
		if ( $filetype eq 'DEITY' ) {
			my $deity = ( $line_ref->{'000DeityName'})[0];
			my $pantheon = "";
			my $sourcepage = "";
		...
		if ( $filetype eq 'EQUIPMENT' ) {
			...
			print { $filehandle_for{EQUIPMENT} }
				qq{"$equipname","$outputname","$sourcepage","$line_for_error","$filename"\n};
		}
...
sub find_full_path {
	my ( $file_name, $current_base_dir, $base_path, $vendor_path ) = @_;
	...
	{
		if ( $file_name !~ s{ ^[&] }{$vendor_path}xmsi ) 
		{
			if ( $file_name !~ s{ ^[*] }{$base_path}xmsi ) 
			{
...

{
	...
	sub parse_system_files {
		...
		open my $csv_file, '>', 'G_export_system.csv';
